      ...
      <Container>
        ...
        {showMenu && (
          <Menu ref={menuRef}>
            ...
            <HoverText
              onClick={() => {
                if (tokensShown + 5 < (tokensForList?.length ?? 0)) setTokensShown(tokensShown + 5)
                else setTokensShown(tokensForList?.length ?? 0)
              }}
              ref={showMoreTokenRef}
              style={{ display: (tokensForList?.length ?? 0) <= tokensShown ? 'none' : "block" }}
            >
              See more...
            </HoverText>
            ...
            <HoverText
              onClick={() => {
                if (poolsShown + 5 < (poolForList?.length ?? 0)) setPoolsShown(poolsShown + 5)
                else setPoolsShown(poolForList?.length ?? 0)
              }}
              ref={showMorePoolRef}
              style={{ display: (poolForList?.length ?? 0) <= poolsShown ? 'none' : 'block' }}
            >
              See more...
            </HoverText>
          </Menu>
