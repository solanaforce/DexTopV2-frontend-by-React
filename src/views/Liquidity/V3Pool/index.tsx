...
import { Currency, CurrencyAmount, Fraction, Percent, Price, Token } from 'libraries/swap-sdk'
import {
  AutoColumn,
  AutoRow,
  Box,
...
import { MasterChefV3, NonfungiblePositionManager, Position } from 'libraries/v3-sdk'
...
export default function PoolPage({parsedTokenId}: {parsedTokenId: bigint | undefined}) {
    ...
    <>
      <LightGreyCard mb="16px">
        <AutoRow justifyContent="space-between" mb="8px">
          ...
          <Text>{feeValueUpper ? formatCurrencyAmount(feeValueUpper, 4, "en-US") : '-'}</Text>
        </AutoRow>
        <AutoRow justifyContent="space-between">
          ...
          <Text>{feeValueLower ? formatCurrencyAmount(feeValueLower, 4, "en-US") : '-'}</Text>
        </AutoRow>
      </LightGreyCard>
    ...
    <TransactionConfirmationModal
      ...
      content={() => (
        <ConfirmationModalContent
          ...
          bottomContent={() => (
            <Button width="100%" onClick={collect} height="48px" variant='primary'>
              Collect
            </Button>
          )}
        />
  ...
  return (
    <Page>
      {/* {!isLoading && <NextSeo title={`${currencyQuote?.symbol}-${currencyBase?.symbol} V3 LP #${parsedTokenId}`} />} */}
				...
				<BodyWrapper>
					{isLoading ? (
						...
						<>
							<AppHeader
								title={
									<Box mb={['8px', '8px', 0]} width="100%" style={{ flex: 1 }} minWidth={['auto', null, 'max-content']}>
										<Flex alignItems="center">
											<DoubleCurrencyLogo size={24} currency0={currencyQuote} currency1={currencyBase} />
											<Heading as="h2" ml="8px">
												{currencyQuote?.symbol}-{currencyBase?.symbol} V3 LP #{parsedTokenId?.toString()}
											</Heading>
											{!isMobile && (
												<>
													{/* {isStakedInMCv3 && (
										...
										{/* <RowBetween gap="16px" flexWrap="nowrap">
											<Text fontSize="14px" color="textSubtle" style={{ wordBreak: 'break-word' }}>
												V3 LP #{parsedTokenId} / {new Percent(feeAmount || 0, 1_000_000).toSignificant()}%{' '}
												fee tier
											</Text>
											{isMobile && (
												<Flex>
                          ...
                          variant='primary'
											...
											>
												<AutoRow justifyContent="space-between" mb="8px">
													...
													<Flex justifyContent="center">
														<Text small>{feeValueUpper ? formatCurrencyAmount(feeValueUpper, 4, "en-US") : '-'}</Text>
													</Flex>
													<RowBetween justifyContent="flex-end">
												...
												<AutoRow justifyContent="space-between">
													...
													<Flex justifyContent="center">
														<Text small>{feeValueLower ? formatCurrencyAmount(feeValueLower, 4, "en-US") : '-'}</Text>
													</Flex>
													<RowBetween justifyContent="flex-end">
														<Text fontSize="10px" color="textSubtle" ml="4px">
								...
								<AutoRow mb="8px">
									...
									>
										<RangePriceSection
											...
											mb={['8px', '8px', '0']}
											title='Min Price'
											price={formatTickPrice(priceLower, tickAtLimit, Bound.LOWER, "en-US")}
										...
										<RangePriceSection
											ml={['0', '0', '16px']}
											title='Max Price'
											price={formatTickPrice(priceUpper, tickAtLimit, Bound.UPPER, "en-US")}
											currency0={currencyQuote}
											currency1={currencyBase}
										/>
									</Flex>
								...
								{pool && currencyQuote && currencyBase ? (
									<RangePriceSection
										...
										currency0={currencyQuote}
										currency1={currencyBase}
										price={formatPrice(inverted ? pool.token1Price : pool.token0Price, 6, "en-US")}
									/>
								) : null}
...
function PositionHistory_({
  ...
  return (
    <AtomBox textAlign="center" pt="16px">
      ...
      {isExpanded && (
        <AtomBox display="grid" gap="16px" mt="20px">
          <AtomBox display="grid" style={{ gridTemplateColumns: '1fr 1fr 1fr' }} alignItems="center">
            <PreTitle>Timestamp</PreTitle>
            <PreTitle>Action</PreTitle>
            <PreTitle>Token Transferred</PreTitle>
          </AtomBox>
...
function PositionHistoryRow({
  ...
  if (isMobile) {
    return (
      <Box>
        <AutoRow gap="8px">
          <LinkExternal href={getBlockExploreLink(positionTx.id, 'transaction', chainId)}>
          ...
          {+positionTx.amount0 > 0 && (
            <AutoRow flexWrap="nowrap" gap="12px" justifyContent="space-between">
              <AutoRow width="auto" flexWrap="nowrap" gap="4px">
                ...
                <Text display={['none', null, 'block']}>{currency0.symbol}</Text>
              </AutoRow>
              <Text ellipsis title={positionTx.amount0}>
                {isPlus ? '+' : '-'}{' '}
          ...
          {+positionTx.amount1 > 0 && (
            <AutoRow flexWrap="nowrap" gap="12px" justifyContent="space-between">
              <AutoRow width="auto" flexWrap="nowrap" gap="4px">
                ...
                <Text display={['none', null, 'block']}>{currency1.symbol}</Text>
              </AutoRow>
              <Text ellipsis title={positionTx.amount1}>
                {isPlus ? '+' : '-'}{' '}
