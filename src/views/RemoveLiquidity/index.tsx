  ...
  const handleSelectCurrencyA = useCallback(
    (currency: Currency) => {
      if (currencyIdB && currencyId(currency) === currencyIdB) {
        router.replace(`/remove/v2?currency=${currencyId(currency)}&currency=${currencyIdA}`, undefined, { shallow: true })
      } else {
        router.replace(`/remove/v2?currency=${currencyId(currency)}&currency=${currencyIdB}`, undefined, { shallow: true })
      }
    },
  ...
  const handleSelectCurrencyB = useCallback(
    (currency: Currency) => {
      if (currencyIdA && currencyId(currency) === currencyIdA) {
        router.replace(`/remove/v2?currency=${currencyIdB}&currency=${currencyId(currency)}`, undefined, { shallow: true })
      } else {
        router.replace(`/remove/v2?currency=${currencyIdA}&currency=${currencyId(currency)}`, undefined, { shallow: true })
      }
    },
    [currencyIdA, currencyIdB, router],
  )
                  ...
                  <CardBody p="8px">
                    ...
                    {chainId && (oneCurrencyIsWNative || oneCurrencyIsNative) ? (
                      <RowBetween style={{ justifyContent: 'flex-end', fontSize: '14px' }}>
                        {oneCurrencyIsNative ? (
                          <InternalLink
                            href={`/remove/v2?currency=${currencyA?.isNative ? WNATIVE[chainId]?.address : currencyIdA}&currency=${
                          ...
                          <InternalLink
                            href={`/remove/v2?currency=${
                              currencyA && currencyA.equals(WNATIVE[chainId]) ? native?.symbol : currencyIdA
                            }&currency=${currencyB && currencyB.equals(WNATIVE[chainId]) ? native?.symbol : currencyIdB}`}
                          >
