...
import { SMART_ROUTER_ADDRESSES, SmartRouterTrade } from 'libraries/smart-router/evm'
...
export const SwapCommitButton = memo(function SwapCommitButton({
  ...
  const routerAddress = SMART_ROUTER_ADDRESSES[trade?.inputAmount?.currency?.chainId ?? ChainId.ETHEREUM]
  ...
  }, [priceImpactWithoutFee, swapCallback, tradeToConfirm, setSwapState, handleConfirmDismiss])
      ...
      <AutoColumn gap="12px">
        ...
        {/* {isRoutingSettingChange && (
          <Message variant="warning" icon={<></>}>
            <AutoColumn gap="8px">
              <MessageText>Unable to establish trading route due to customized routing.</MessageText>
              <AutoRow gap="4px">
    ...
    <>
      <RowBetween>
        ...
        >
          {(tradeLoading && (
            <>
              <Dots>Searching For The Best Price</Dots>
            </>
          )) ||
            (priceImpactSeverity > 3 && !isExpertMode
              ? 'Price Impact High'
