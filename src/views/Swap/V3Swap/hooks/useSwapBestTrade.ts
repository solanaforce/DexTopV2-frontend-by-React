import { CurrencyAmount, ERC20Token, TradeType } from 'libraries/swap-sdk'
import tryParseAmount from 'utils/tryParseAmount'
import { useUserSingleHopOnly } from 'utils/user'
...
export function useSwapBestTrade({ maxHops }: Options = {}) {
  ...
  const { isLoading, trade, refresh, syncing, isStale, error } = useBestAMMTrade({
    amount,
    currency: dependentCurrency ?? undefined,
    baseCurrency: independentCurrency ?? undefined,
    tradeType,
