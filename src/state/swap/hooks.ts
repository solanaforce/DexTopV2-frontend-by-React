...
import { GTOKEN, USDT, USDT_ETH } from 'libraries/tokens'
...
export function queryParametersToSwapState(
  ...
  let inputCurrency = safeGetAddress(parsedQs.inputCurrency) || (nativeSymbol ?? "PLS")
  let outputCurrency =
    typeof parsedQs.outputCurrency === 'string'
      ? safeGetAddress(parsedQs.outputCurrency) || nativeSymbol
      : defaultOutputCurrency ?? USDT_ETH.address
...
export function useDefaultsFromURLSearch():
  ...
  useEffect(() => {
    ...
    if (!GTOKEN[chainId]) return
    const parsed = queryParametersToSwapState(query, native.symbol, GTOKEN[chainId]?.address)
