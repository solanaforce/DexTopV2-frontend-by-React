...
import { CurrencyAmount, TradeType, Currency } from 'libraries/swap-sdk'
...
import { usePropsChanged } from './usePropsChanged'
...
function bestTradeHookFactory({
  ...
  return function useBestTrade({
    ...
    const poolProvider = useMemo(() => SmartRouter.createStaticPoolProvider(candidatePools ?? undefined), [candidatePools])
    ...
    const {
      ...
      retry: false,
      staleTime: autoRevalidate ? POOLS_NORMAL_REVALIDATE[amount?.currency?.chainId ?? ChainId.ETHEREUM] : 0,
      refetchInterval: autoRevalidate && POOLS_NORMAL_REVALIDATE[amount?.currency?.chainId ?? ChainId.ETHEREUM],
    })
